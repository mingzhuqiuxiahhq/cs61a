...
#        assert (message == 'withdraw') or (message == 'deposit'), 'Invalid message'
        if message == 'withdraw':
            if balance >= amount:
                balance -= amount
                return balance
...
def make_withdraw(balance, password):
    ...
    password_attempt = 0
    wrong_password = []
    def password_withdraw(amount, match_password):
        nonlocal balance, password, password_attempt, wrong_password
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    "Too many incorrect attempts. Attempts: ['my', 'secret', 'password']"
    """
    new_balance = withdraw(0,old_pass)
    if type(new_balance) == str:
        return new_balance
