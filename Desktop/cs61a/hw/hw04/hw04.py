...
def balanced(m):
    ...
    True
    """
    #get arm length and total weight of each arm
...
def replace_leaf(t, find_value, replace_value):
    ...
    True
    """
    if is_tree(t) and label(t) == find_value and len(branches(t)) ==1:
...
#        return [label(t)] + [preorder(b) + [] for b in branches(t)]
    ...
    flat_branches = []
    for b in branches(t):
        flat_branches += preorder(b)
    return [label(t)] + flat_branches
...
#        return -b/(2*a)
#    for t in x:
#        return interval(min(f(x,a,b,c),f_derv(x,a,b),extreme(a,b)) ,max(f(x,a,b,c),f_derv(x,a,b),extreme(a,b)))


    f  = lambda t: t*a*t + b*t + c
    extreme = -b/(2*a)
    l,u, e= f(lower_bound(x)), f(upper_bound(x)), f(extreme)
...
def square_tree(t):
    ...
    if len(t) > 1:
        for b in t:
            return tree(b*b, square_tree(t[1:]))
    else:
